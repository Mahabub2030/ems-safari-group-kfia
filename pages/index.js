/* ============================================================
   DATA GENERATION - Safari Contracting Company
   ============================================================ */
...
const documentsList = [
  { title: "Safari Commercial Registration (CR)", category: "Legal & Corporate", date: "2025-01-15", size: "2.4 MB", img: "https://images.unsplash.com/photo-1450133064473-71024230f91b?auto=format&fit=crop&w=600&q=80", desc: "Official corporate commercial registration certificate." },
  { title: "Aramco Vendor Safety Compliance", category: "Compliance", date: "2026-03-10", size: "4.1 MB", img: "https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?auto=format&fit=crop&w=600&q=80", desc: "Certified Aramco contractor safety assessment." },
  { title: "GOSI Insurance Policy Certificate", category: "Insurance", date: "2026-01-01", size: "1.8 MB", img: "https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?auto=format&fit=crop&w=600&q=80", desc: "GOSI coverage certificate for all workforce." },
  { title: "Project Specs & Blueprint", category: "Engineering", date: "2026-06-28", size: "5.5 MB", img: "https://images.unsplash.com/photo-1503387762-592deb58ef4e?auto=format&fit=crop&w=600&q=80", desc: "Technical specifications and master blueprints." },
  { title: "Standard Employment Contract", category: "HR & Legal", date: "2026-05-12", size: "850 KB", img: "https://images.unsplash.com/photo-1589829545856-d10d557cf95f?auto=format&fit=crop&w=600&q=80", desc: "Saudi Labor Law compliant employment contract." },
  { title: "MOL Work Permit Clearances", category: "Government", date: "2026-06-01", size: "3.2 MB", img: "https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?auto=format&fit=crop&w=600&q=80", desc: "Ministry of Labor batch work permit approvals." },
  { title: "ISO 9001 Quality Certification", category: "Compliance", date: "2024-11-20", size: "1.9 MB", img: "https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?auto=format&fit=crop&w=600&q=80", desc: "Quality management systems certification." },
  { title: "Dammam Yard Safety Protocol", category: "Safety (HSE)", date: "2026-02-14", size: "1.1 MB", img: "https://images.unsplash.com/photo-1541888946425-d0fbb18f7663?auto=format&fit=crop&w=600&q=80", desc: "Occupational health and safety rulebook." }
];
...
function switchPage(pageId){
  ...
  if (pageId === "dashboard") setTimeout(renderCharts, 50);
  else if (pageId === "documentation") setTimeout(renderDocBarChart, 50);
}
...
function renderDashboard(){
  ...
  const kpis = [
    ...
    { title: "Critical Iqamas", val: critical, sub: "Expiring ≤14 Days / Expired", icon: "alert-triangle", color: "text-rose-600", bg: "bg-rose-50" },
  ...
  if (urgent.length === 0){
    alertContainer.innerHTML = `<p class="text-sm text-slate-500 col-span-4 py-3">No urgent Iqama renewals requiring immediate action (≤14 days).</p>`;
  } else {
    alertContainer.innerHTML = urgent.slice(0,4).map(e => `
      <div class="border border-rose-200 bg-rose-50/50 rounded-xl p-4 flex flex-col justify-between">
        ...
        <div class="mt-4 pt-2 border-t border-rose-100 flex items-center justify-between">
          ...
          <button onclick="openIdCard('${e.id}')" class="text-xs font-semibold text-purple-600 hover:underline">View ID Card →</button>
        </div>
      </div>
    `).join("");
  }
...
function renderCharts(){
  ...
  natChartInstance = new Chart(document.getElementById("nationalityChart").getContext("2d"), {
    type: "doughnut",
    data: {
      labels: [...topNats.map(x=>x[0]), "Other Expat"],
      datasets: [{ data: [...topNats.map(x=>x[1]), otherCount], backgroundColor: ["#7c3aed","#3b82f6","#10b981","#f59e0b","#ef4444","#94a3b8"], borderWidth: 2, borderColor: "#fff" }]
    },
  ...
  // Policy & Compliance
  policyChartInstance = new Chart(document.getElementById("policyChart").getContext("2d"), {
    type: "doughnut",
    data: {
      labels: ["Valid Iqama", "Expiring Soon", "Critical", "Expired", "On Leave"],
      datasets: [{
        data: [
          employees.filter(e => e.status === "Valid").length,
          employees.filter(e => e.status === "Expiring Soon").length,
          employees.filter(e => e.status === "Critical").length,
          employees.filter(e => e.status === "Expired").length,
          employees.filter(e => e.dutyStatus === "On Leave").length
        ],
        backgroundColor: ["#10b981", "#f59e0b", "#ef4444", "#7c3aed", "#3b82f6"],
...
function renderIqamaPage(){
  ...
  const cards = [
    ...
    { label: "Expiring Soon (15-30)", count: expiringSoon, filter: "Expiring Soon", color: "border-amber-200 bg-amber-50/50 text-amber-900" },
  ...
  document.getElementById("iqamaFilterPills").innerHTML = pills.map(p => `
    <button onclick="setIqamaTab('${p.id}')" class="px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${iqamaTab === p.id ? 'bg-purple-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}">${p.label}</button>
  `).join("");
  ...
  if (paginated.length === 0){
    tbody.innerHTML = `<tr><td colspan="7" class="text-center py-8 text-slate-400">No matching records found.</td></tr>`;
  } else {
    tbody.innerHTML = paginated.map(e => {
      let badge = e.status === "Valid" ? "bg-emerald-100 text-emerald-800" :
                  e.status === "Expiring Soon" ? "bg-amber-100 text-amber-800" :
                  e.status === "Critical" ? "bg-rose-100 text-rose-800" : "bg-purple-100 text-purple-800";
      return `
        <tr class="hover:bg-slate-50/80">
          <td class="px-4 py-3"><div class="font-medium text-slate-900">${e.name}</div><div class="text-xs text-slate-500">${e.id} • ${e.craft}</div></td>
          ...
          <td class="px-4 py-3 mono-num font-semibold ${e.daysRemaining < 0 ? 'text-purple-600' : e.daysRemaining <= 14 ? 'text-rose-600' : 'text-slate-800'}">${e.daysRemaining < 0 ? Math.abs(e.daysRemaining)+"d overdue" : e.daysRemaining+" days"}</td>
          <td class="px-4 py-3"><span class="px-2 py-0.5 rounded-full text-xs font-semibold ${badge}">${e.status}</span></td>
          <td class="px-4 py-3"><button onclick="openIdCard('${e.id}')" class="px-2.5 py-1 rounded-md bg-purple-50 text-purple-700 text-xs font-semibold hover:bg-purple-100">ID Card</button></td>
        </tr>`;
    }).join("");
  }

  document.getElementById("iqamaPagination").innerHTML = `
    <span>Showing ${filtered.length ? start+1 : 0} to ${Math.min(start+iqamaPageSize, filtered.length)} of ${filtered.length}</span>
    <div class="flex gap-1">
      <button onclick="changeIqamaPage(${iqamaPage-1})" ${iqamaPage<=1?'disabled class="opacity-40 px-3 py-1 rounded border"':'class="px-3 py-1 rounded border hover:bg-slate-50"'}>Prev</button>
      <span class="px-3 py-1 font-medium">${iqamaPage} / ${totalPages}</span>
      <button onclick="changeIqamaPage(${iqamaPage+1})" ${iqamaPage>=totalPages?'disabled class="opacity-40 px-3 py-1 rounded border"':'class="px-3 py-1 rounded border hover:bg-slate-50"'}>Next</button>
    </div>`;
  safeCreateIcons();
}

function setIqamaTab(tab){ iqamaTab = tab; iqamaPage = 1; renderIqamaPage(); }
function changeIqamaPage(p){ iqamaPage = p; renderIqamaPage(); }
...
function renderOrgPage(){
  document.getElementById("orgChart").innerHTML = `
    <div class="flex flex-col items-center min-w-[700px] py-2">
      ...
      <div class="grid grid-cols-4 gap-4 w-full px-4">
        <div class="bg-white border border-slate-200 rounded-lg p-3 text-center shadow-sm"><p class="text-xs font-bold text-slate-700">Operations & Maintenance</p><p class="text-[11px] text-slate-400 mt-0.5">Field Execution</p></div>
        <div class="bg-white border border-slate-200 rounded-lg p-3 text-center shadow-sm"><p class="text-xs font-bold text-slate-700">Safety & HSE</p><p class="text-[11px] text-slate-400 mt-0.5">Compliance & Audits</p></div>
        <div class="bg-white border border-slate-200 rounded-lg p-3 text-center shadow-sm"><p class="text-xs font-bold text-slate-700">Engineering & Technical</p><p class="text-[11px] text-slate-400 mt-0.5">Design & QA/QC</p></div>
        <div class="bg-white border border-slate-200 rounded-lg p-3 text-center shadow-sm"><p class="text-xs font-bold text-slate-700">Admin & HR</p><p class="text-[11px] text-slate-400 mt-0.5">Workforce Management</p></div>
      </div>
    </div>`;
  ...
  document.getElementById("shiftCards").innerHTML = Object.entries(shiftCounts).map(([sh, count]) => `
    <div class="bg-white border border-slate-200 rounded-xl p-4 flex items-center justify-between">
      <div><p class="text-xs font-semibold text-slate-500">${sh}</p><p class="text-xl font-bold text-slate-900 mt-1 mono-num">${count} <span class="text-xs font-normal text-slate-400">Personnel</span></p></div>
      <div class="w-10 h-10 rounded-lg ${shiftColors[sh].bg} ${shiftColors[sh].color} flex items-center justify-center"><i data-lucide="${shiftColors[sh].icon}" class="w-5 h-5"></i></div>
    </div>`).join("");
  ...
  document.getElementById("crewTableBody").innerHTML = employees.slice(0,10).map(e => `
    <tr class="hover:bg-slate-50">
      <td class="px-4 py-3 font-medium text-slate-900">${e.name} <span class="text-xs text-slate-400">(${e.id})</span></td>
...
function renderManpowerPage(){
  ...
  const colLabels = { id:"Emp ID", name:"Name", nationality:"Nationality", craft:"Craft", department:"Department", iqama:"Iqama No.", expiryDate:"Expiry Date", dutyStatus:"Duty Status", jobSite:"Job Site", actions:"Actions" };
  document.getElementById("colToggleMenu").innerHTML = Object.entries(mpColumns).map(([key, val]) => `
    <label class="flex items-center gap-2 px-2 py-1 hover:bg-slate-50 rounded cursor-pointer">
      <input type="checkbox" ${val?'checked':''} onchange="toggleMpCol('${key}')">
  ...
  document.getElementById("mpTableHead").innerHTML = headHtml;

  const tbody = document.getElementById("mpTableBody");
  if (paginated.length === 0){
    tbody.innerHTML = `<tr><td colspan="10" class="text-center py-8 text-slate-400">No matching records.</td></tr>`;
  } else {
    tbody.innerHTML = paginated.map(e => {
      ...
      let row = `<tr class="hover:bg-slate-50/80">`;
      if (mpColumns.id) row += `<td class="px-4 py-3 font-mono text-xs text-slate-600">${e.id}</td>`;
      if (mpColumns.name) row += `<td class="px-4 py-3 font-medium text-slate-900">${e.name}</td>`;
      if (mpColumns.nationality) row += `<td class="px-4 py-3 text-slate-700">${nationalityFlags[e.nationality]||""} ${e.nationality}</td>`;
      if (mpColumns.craft) row += `<td class="px-4 py-3 text-slate-700">${e.craft}</td>`;
      if (mpColumns.department) row += `<td class="px-4 py-3 text-slate-600">${e.department}</td>`;
      if (mpColumns.iqama) row += `<td class="px-4 py-3 font-mono text-slate-700">${e.iqama}</td>`;
      if (mpColumns.expiryDate) row += `<td class="px-4 py-3 text-slate-600">${fmtDate(e.expiryDate)}</td>`;
      if (mpColumns.dutyStatus) row += `<td class="px-4 py-3"><span class="px-2 py-0.5 rounded-full text-xs font-semibold ${dutyBadge}">${e.dutyStatus}</span></td>`;
      if (mpColumns.jobSite) row += `<td class="px-4 py-3 text-slate-600 text-xs">${e.jobSite}</td>`;
      if (mpColumns.actions) row += `<td class="px-4 py-3"><button onclick="openIdCard('${e.id}')" class="px-2 py-1 bg-purple-50 text-purple-700 rounded text-xs font-semibold hover:bg-purple-100">ID Card</button></td>`;
      row += `</tr>`;
      return row;
    }).join("");
  }

  document.getElementById("mpCardView").innerHTML = paginated.map(e => `
    <div class="bg-white border border-slate-200 rounded-xl p-4 flex flex-col justify-between shadow-sm">
      <div>
        <div class="flex justify-between"><span class="text-xs font-mono text-slate-400">${e.id}</span>
        <span class="px-2 py-0.5 rounded-full text-[10px] font-semibold ${e.dutyStatus==='On Duty'?'bg-emerald-100 text-emerald-800':'bg-amber-100 text-amber-800'}">${e.dutyStatus}</span></div>
        ...
        <div class="mt-3 space-y-1 text-xs text-slate-600">
          <p>Nationality: ${nationalityFlags[e.nationality]||""} ${e.nationality}</p>
          <p>Iqama: <span class="font-mono">${e.iqama}</span></p>
          <p>Job Site: ${e.jobSite}</p>
        </div>
      </div>
  ...
  document.getElementById("mpPagination").innerHTML = `
    <span>Showing ${filtered.length?start+1:0} to ${Math.min(start+mpPageSize,filtered.length)} of ${filtered.length}</span>
    <div class="flex gap-1">
      <button onclick="changeMpPage(${mpPage-1})" ${mpPage<=1?'disabled class="opacity-40 px-3 py-1 rounded border"':'class="px-3 py-1 rounded border hover:bg-slate-50"'}>Prev</button>
      <span class="px-3 py-1 font-medium">${mpPage} / ${totalPages}</span>
      <button onclick="changeMpPage(${mpPage+1})" ${mpPage>=totalPages?'disabled class="opacity-40 px-3 py-1 rounded border"':'class="px-3 py-1 rounded border hover:bg-slate-50"'}>Next</button>
    </div>`;
  safeCreateIcons();
}

function toggleMpCol(key){ mpColumns[key] = !mpColumns[key]; renderManpowerPage(); }
...
function renderEquipmentPage(){
  ...
  document.getElementById("equipKpis").innerHTML = [
    { title:"Total Assets", val:equipmentList.length, color:"text-purple-600", bg:"bg-purple-50" },
    { title:"In Use", val:inUse, color:"text-emerald-600", bg:"bg-emerald-50" },
    { title:"Available", val:available, color:"text-blue-600", bg:"bg-blue-50" },
    { title:"Under Maintenance", val:maintenance, color:"text-amber-600", bg:"bg-amber-50" }
  ].map(k => `
    <div class="bg-white border border-slate-200 rounded-xl p-4 flex items-center justify-between">
      <div><p class="text-xs font-semibold text-slate-500 uppercase">${k.title}</p><p class="text-2xl font-bold text-slate-900 mt-1 mono-num">${k.val}</p></div>
      <div class="w-10 h-10 rounded-lg ${k.bg} ${k.color} flex items-center justify-center"><i data-lucide="wrench" class="w-5 h-5"></i></div>
    </div>`).join("");

  const pills = [{id:"all",label:"All"},{id:"In Use",label:"In Use"},{id:"Available",label:"Available"},{id:"Under Maintenance",label:"Maintenance"},{id:"Retired",label:"Retired"}];
  document.getElementById("equipFilterPills").innerHTML = pills.map(p => `
    <button onclick="setEquipTab('${p.id}')" class="px-3 py-1.5 rounded-lg text-xs font-medium ${equipTab===p.id?'bg-purple-600 text-white':'bg-slate-100 text-slate-600 hover:bg-slate-200'}">${p.label}</button>`).join("");
  ...
  document.getElementById("equipTableBody").innerHTML = paginated.length === 0
    ? `<tr><td colspan="7" class="text-center py-8 text-slate-400">No equipment found.</td></tr>`
    : paginated.map(eq => {
        ...
        return `<tr class="hover:bg-slate-50">
          ...
          <td class="px-4 py-3"><span class="px-2 py-0.5 rounded bg-slate-100 text-xs font-semibold">${eq.category}</span></td>
  ...
  document.getElementById("equipPagination").innerHTML = `
    <span>Showing ${filtered.length?start+1:0} to ${Math.min(start+equipPageSize,filtered.length)} of ${filtered.length}</span>
    <div class="flex gap-1">
      <button onclick="changeEquipPage(${equipPage-1})" ${equipPage<=1?'disabled class="opacity-40 px-3 py-1 rounded border"':'class="px-3 py-1 rounded border hover:bg-slate-50"'}>Prev</button>
      <span class="px-3 py-1 font-medium">${equipPage} / ${totalPages}</span>
      <button onclick="changeEquipPage(${equipPage+1})" ${equipPage>=totalPages?'disabled class="opacity-40 px-3 py-1 rounded border"':'class="px-3 py-1 rounded border hover:bg-slate-50"'}>Next</button>
    </div>`;
  safeCreateIcons();
}
...
function renderVacationPage(){
  ...
  document.getElementById("leaveKpis").innerHTML = `
    <div class="bg-white border border-slate-200 rounded-xl p-4 flex items-center justify-between">
      <div><p class="text-xs font-semibold text-slate-500 uppercase">On Leave</p><p class="text-2xl font-bold text-slate-900 mt-1 mono-num">${onLeaveCount}</p></div>
    ...
    <div class="bg-white border border-slate-200 rounded-xl p-4 flex items-center justify-between">
      <div><p class="text-xs font-semibold text-slate-500 uppercase">Returning ≤14 Days</p><p class="text-2xl font-bold text-slate-900 mt-1 mono-num">${returningCount}</p></div>
  ...
  document.getElementById("ganttChart").innerHTML = vacationRequests.slice(0,6).map(v => `
    <div class="flex items-center gap-4 text-xs">
      ...
      <div class="flex-1 bg-slate-100 h-6 rounded-md relative overflow-hidden flex items-center px-2">
        ...
        <span class="relative z-10 text-[11px] font-medium text-slate-700">${fmtDate(v.start)} → ${fmtDate(v.end)} (${v.leaveType})</span>
      </div>
    </div>`).join("");

  let filtered = vacationRequests.filter(v => !vacSearchVal || v.name.toLowerCase().includes(vacSearchVal.toLowerCase()) || v.id.toLowerCase().includes(vacSearchVal.toLowerCase()));
  ...
  document.getElementById("vacTableBody").innerHTML = paginated.length === 0
    ? `<tr><td colspan="7" class="text-center py-8 text-slate-400">No requests found.</td></tr>`
    : paginated.map(v => {
        ...
        return `<tr class="hover:bg-slate-50">
          <td class="px-4 py-3 font-medium text-slate-900">${v.name} <span class="text-xs text-slate-400">(${v.id})</span></td>
  ...
  document.getElementById("vacPagination").innerHTML = `
    <span>Showing ${filtered.length?start+1:0} to ${Math.min(start+vacPageSize,filtered.length)} of ${filtered.length}</span>
    <div class="flex gap-1">
      <button onclick="changeVacPage(${vacPage-1})" ${vacPage<=1?'disabled class="opacity-40 px-3 py-1 rounded border"':'class="px-3 py-1 rounded border hover:bg-slate-50"'}>Prev</button>
      <span class="px-3 py-1 font-medium">${vacPage} / ${totalPages}</span>
      <button onclick="changeVacPage(${vacPage+1})" ${vacPage>=totalPages?'disabled class="opacity-40 px-3 py-1 rounded border"':'class="px-3 py-1 rounded border hover:bg-slate-50"'}>Next</button>
    </div>`;
  safeCreateIcons();
}
...
function renderDocumentationPage(){
  docSearchVal = document.getElementById("docSearch")?.value || "";
  let filteredDocs = documentsList.filter(doc => !docSearchVal || doc.title.toLowerCase().includes(docSearchVal.toLowerCase()) || doc.category.toLowerCase().includes(docSearchVal.toLowerCase()));

  const pageSize = docRowsPerPage === "all" ? filteredDocs.length : (parseInt(docRowsPerPage,10) || 5);
  ...
  const startIndex = (docCurrentPage - 1) * pageSize;
  const currentDocs = docRowsPerPage === "all" ? filteredDocs : filteredDocs.slice(startIndex, startIndex + pageSize);

  document.getElementById("docTableBody").innerHTML = currentDocs.length === 0
    ? `<tr><td colspan="5" class="px-4 py-8 text-center text-slate-500">No documents found.</td></tr>`
    : currentDocs.map(doc => `
      <tr class="hover:bg-slate-50">
        <td class="px-4 py-3 font-medium text-slate-900 flex items-center gap-2">
          <div class="w-8 h-8 rounded bg-purple-50 text-purple-600 flex items-center justify-center shrink-0"><i data-lucide="file-text" class="w-4 h-4"></i></div>
          <span>${doc.title}</span>
        </td>
        <td class="px-4 py-3"><span class="px-2.5 py-1 text-xs font-semibold bg-purple-50 text-purple-700 rounded-full">${doc.category}</span></td>
        <td class="px-4 py-3 text-slate-600">${doc.date}</td>
        <td class="px-4 py-3 text-slate-600">${doc.size}</td>
        <td class="px-4 py-3 text-right">
          <button onclick="openDocModal('${doc.title.replace(/'/g,"\\'")}','${doc.img}','${doc.desc.replace(/'/g,"\\'")}')" class="px-3 py-1.5 bg-purple-50 hover:bg-purple-100 text-purple-700 text-xs font-semibold rounded-lg inline-flex items-center gap-1.5">
            <i data-lucide="eye" class="w-3.5 h-3.5"></i> Preview
          </button>
        </td>
      </tr>`).join("");

  const startNum = filteredDocs.length ? startIndex + 1 : 0;
  const endNum = docRowsPerPage === "all" ? filteredDocs.length : Math.min(startIndex + pageSize, filteredDocs.length);
  document.getElementById("docPaginationInfo").textContent = `Showing ${startNum} to ${endNum} of ${filteredDocs.length}`;

  document.getElementById("docPaginationButtons").innerHTML = `
    <button onclick="changeDocPage(${docCurrentPage-1})" ${docCurrentPage<=1?'disabled class="opacity-40 px-3 py-1 rounded border bg-white"':'class="px-3 py-1 rounded border bg-white hover:bg-slate-50"'}>Prev</button>
    <span class="px-3 py-1 font-medium">${docCurrentPage} / ${totalPages||1}</span>
    <button onclick="changeDocPage(${docCurrentPage+1})" ${docCurrentPage>=totalPages?'disabled class="opacity-40 px-3 py-1 rounded border bg-white"':'class="px-3 py-1 rounded border bg-white hover:bg-slate-50"'}>Next</button>`;
  safeCreateIcons();
}

function changeDocPage(p){ docCurrentPage = p; renderDocumentationPage(); }

function renderDocBarChart(){
  if (docBarChartInstance) docBarChartInstance.destroy();
  const categoryCounts = {};
  documentsList.forEach(d => categoryCounts[d.category] = (categoryCounts[d.category]||0)+1);
  docBarChartInstance = new Chart(document.getElementById("docBarChart").getContext("2d"), {
    type: "bar",
    data: { labels: Object.keys(categoryCounts), datasets: [{ label:"Documents", data: Object.values(categoryCounts), backgroundColor:"#7c3aed", borderRadius:6, barThickness:36 }] },
    options: { responsive:true, maintainAspectRatio:false, plugins:{legend:{display:false}}, scales:{ x:{grid:{display:false}}, y:{grid:{color:"#f1f5f9"}, ticks:{precision:0}} } }
  });
}
  ...
  document.getElementById("idCardContent").innerHTML = `
    <div class="bg-gradient-to-br from-slate-900 via-purple-950 to-slate-900 text-white p-6 relative overflow-hidden">
      ...
      <div class="flex items-center justify-between border-b border-white/10 pb-4">
        <div><p class="text-[10px] font-bold tracking-widest text-purple-400 uppercase">Safari Contracting Company</p><p class="text-xs text-slate-300">Official Personnel ID Card</p></div>
        <div class="w-8 h-8 rounded bg-purple-600 flex items-center justify-center font-bold text-xs">S</div>
      </div>
      <div class="mt-6 flex items-center gap-4">
        <div class="w-16 h-16 rounded-xl bg-purple-600/30 border border-purple-500/40 flex items-center justify-center font-bold text-xl text-purple-200 shrink-0">${e.name.split(" ").map(n=>n[0]).join("").slice(0,2)}</div>
        <div><h3 class="font-bold text-base text-white leading-tight">${e.name}</h3><p class="text-xs text-purple-300 mt-0.5">${e.craft}</p><p class="text-[11px] font-mono text-slate-400 mt-1">${e.id}</p></div>
      </div>
      ...
      <div class="mt-6 pt-4 border-t border-white/10 flex items-center justify-between">
        <div class="bg-white p-2 rounded-lg"><div class="w-12 h-12 bg-slate-900 text-white flex items-center justify-center text-[9px] font-mono text-center leading-none p-1">QR CODE</div></div>
...
function setupEventListeners(){
  document.getElementById("iqamaSearch")?.addEventListener("input", e => { iqamaSearchVal = e.target.value; iqamaPage = 1; renderIqamaPage(); });

  document.getElementById("colToggleBtn")?.addEventListener("click", e => { e.stopPropagation(); document.getElementById("colToggleMenu").classList.toggle("hidden"); });
  document.addEventListener("click", () => document.getElementById("colToggleMenu")?.classList.add("hidden"));
  ...
  document.getElementById("exportExcelBtn")?.addEventListener("click", () => {
    ...
    const data = filtered.map(e => ({ "Emp ID":e.id, "Full Name":e.name, "Nationality":e.nationality, "Craft":e.craft, "Department":e.department, "Iqama No.":e.iqama, "Expiry Date":fmtDate(e.expiryDate), "Duty Status":e.dutyStatus, "Job Site":e.jobSite }));
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Manpower");
    XLSX.writeFile(wb, "Safari_Contracting_Manpower.xlsx");
  });

  document.getElementById("exportPdfBtn")?.addEventListener("click", () => {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF("l", "mm", "a4");
    doc.setFontSize(16); doc.setTextColor(76,29,149);
    doc.text("Safari Contracting Company - Manpower Directory", 14, 20);
    doc.setFontSize(10); doc.setTextColor(100,116,139);
    doc.text(`Generated: ${fmtDate(today)} | Dammam, Saudi Arabia`, 14, 27);
    let filtered = employees.filter(e => {
      if (mpDeptVal !== "all" && e.department !== mpDeptVal) return false;
      if (mpCraftVal !== "all" && e.craft !== mpCraftVal) return false;
      if (mpStatusVal !== "all" && e.dutyStatus !== mpStatusVal) return false;
    ...
    doc.autoTable({
      ...
      body: filtered.map(e => [e.id, e.name, e.nationality, e.craft, e.department, e.iqama, fmtDate(e.expiryDate), e.dutyStatus]),
  ...
  document.getElementById("docSearch")?.addEventListener("input", () => { docCurrentPage = 1; renderDocumentationPage(); });
  document.getElementById("docRowsPerPage")?.addEventListener("change", e => { docRowsPerPage = e.target.value; docCurrentPage = 1; renderDocumentationPage(); });

  document.getElementById("idCardCloseBtn")?.addEventListener("click", () => document.getElementById("idCardModal").classList.add("hidden"));
  document.getElementById("docCloseBtn")?.addEventListener("click", () => document.getElementById("docModal").classList.add("hidden"));
  document.getElementById("docDownloadBtn")?.addEventListener("click", () => { alert("File download initiated successfully."); document.getElementById("docModal").classList.add("hidden"); });

  window.addEventListener("click", e => {
    if (e.target === document.getElementById("idCardModal")) document.getElementById("idCardModal").classList.add("hidden");
    if (e.target === document.getElementById("docModal")) document.getElementById("docModal").classList.add("hidden");
  });
}
